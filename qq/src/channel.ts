    ...
    try {
        ...
        if (payloads.length > 1) {
            ...
            if (allMediaPaths.length > 0) {
                ...
                mergedCtx.MediaPath = allMediaPaths[0];
            }
            const allMediaTypes = payloads.flatMap(p => p.ctxPayload.MediaTypes || []);
            if (allMediaTypes.length > 0) {
                mergedCtx.MediaTypes = allMediaTypes;
            }
        ...
        await payloads[0].executeDispatch(mergedCtx, {
            isStale: () => {
                ...
                // When interruptOnNewMessage is disabled, only check if this run
                // was replaced (activeEpoch changed). Do NOT abort just because a
                // new message arrived (latestEpoch changed) — let the current
                // dispatch finish and the new message will be processed afterward.
                if (config.interruptOnNewMessage === false) {
                    return state.activeEpoch !== runEpoch;
                }
        ...
        q.activeEpoch = 0;
        console.log(`[QQ][DRAIN:done] sessionKey=${sessionKey} remainingInQueue=${q.pendingPayloads.length}`);
        if (q.pendingPayloads.length > 0 && !q.timer) {
            console.log(`[QQ][DRAIN:continue] Re-draining ${q.pendingPayloads.length} pending message(s) for sessionKey=${sessionKey}`);
...
function enqueueQQMessageForDispatch(sessionKey: string, msg: PendingQQMsg, config: QQConfig, sendMessageFn: (msg: string) => void) {
    ...
    if (!q) {
        q = { pendingPayloads: [], timer: null, isProcessing: false, latestEpoch: 0, activeEpoch: 0, lastDrainStartTime: 0 };
        sessionQueues.set(sessionKey, q);
    }

    // Rate limit: only accept one message per session cooldown window.
    // Drop if: already processing, already queued (debounce pending), or in cooldown.
    const rateLimitMs = (config as any).sessionCooldownMs ?? 20000;
    const elapsed = Date.now() - q.lastDrainStartTime;
    const busy = q.isProcessing || q.pendingPayloads.length > 0;
    const inCooldown = q.lastDrainStartTime > 0 && elapsed < rateLimitMs;
    if (busy || inCooldown) {
        const reason = q.isProcessing ? "processing"
            : q.pendingPayloads.length > 0 ? "queued (debounce pending)"
            : `cooldown (${Math.ceil((rateLimitMs - elapsed) / 1000)}s left)`;
        console.log(`[QQ][DROP:rate-limit] sessionKey=${sessionKey} reason=${reason}`);
        return;
    }
    ...
    console.log(`[QQ][ENQUEUE] sessionKey=${sessionKey} queueLen=${q.pendingPayloads.length} isProcessing=${q.isProcessing} epoch=${q.latestEpoch} debounceMs=${debounceMs}`);
...
// ───── GPT-SoVITS TTS helper ─────
import http from "node:http";

async function generateSoVITSAudio(
    text: string,
    config: QQConfig,
    logger?: { info: (...a: any[]) => void; warn: (...a: any[]) => void; error: (...a: any[]) => void },
): Promise<string | null> {
    const log = logger || { info: console.log, warn: console.warn, error: console.error };
    if (!config.enableSoVITS) return null;
    if (!config.sovitsRefAudioPath) { log.warn("[SoVITS] No reference audio configured"); return null; }

    // Strip CQ codes, markdown formatting, and whitespace
    let cleanText = text
        .replace(/\[CQ:.*?\]/g, "")          // CQ codes
        .replace(/\*\*(.+?)\*\*/g, "$1")     // **bold**
        .replace(/\*(.+?)\*/g, "$1")         // *italic*
    ...
    const maxChars = config.sovitsMaxChars || 300;
    if (cleanText.length > maxChars) {
        ...
        cleanText = lastSentEnd > maxChars * 0.3 ? truncated.slice(0, lastSentEnd + 1) : truncated;
        log.info(`[SoVITS] Text truncated: ${text.length} -> ${cleanText.length} chars`);
    }
    if (!cleanText) return null;

    try {
        const apiUrl = new URL("/tts", config.sovitsApiUrl || "http://127.0.0.1:9880");
        const body = JSON.stringify({
            text: cleanText,
            text_lang: "auto",
            ref_audio_path: config.sovitsRefAudioPath,
            prompt_text: config.sovitsPromptText || "",
            prompt_lang: config.sovitsPromptLang || "ja",
            media_type: "wav",
        ...
        const audioBuffer = await new Promise<Buffer>((resolve, reject) => {
                ...
                (res) => {
                    ...
                    res.on("end", () => {
                        ...
                        if (res.statusCode !== 200) {
                            reject(new Error(`SoVITS API ${res.statusCode}: ${buf.toString("utf-8").slice(0, 200)}`));
                        } else {
                            resolve(buf);
                        }
                    });
            ...
            req.on("error", reject);
            req.on("timeout", () => { req.destroy(); reject(new Error("SoVITS API timeout")); });
            req.write(body);
            req.end();
        });

        if (audioBuffer.length < 100) {
            log.warn(`[SoVITS] Audio too small (${audioBuffer.length} bytes), likely error`);
            return null;
        }

        const b64 = audioBuffer.toString("base64");
        log.info(`[SoVITS] Generated audio: ${audioBuffer.length} bytes, base64 ${b64.length} chars for "${cleanText.slice(0, 30)}..."`);
        return `[CQ:record,file=base64://${b64}]`;
    } catch (err: any) {
        log.error(`[SoVITS] TTS failed: ${err.message || err}`);
        return null;
    }
}
...
async function grokDrawDirect(prompt: string): Promise<{ ok: true; url: string } | { ok: false; error: string }> {
    ...
    const apiKey = process.env.GROK2API_KEY || "";
    if (!apiKey) return { ok: false, error: "未配置 GROK2API_KEY 环境变量" };
...
function isImageFile(url: string): boolean {
    ...
    let clean = url;
    try { clean = new URL(url).pathname; } catch { /* not a valid URL, check as-is */ }
...
/** Extract image URLs from model output text and return { cleanText, imageUrls } */
function extractImageUrlsFromText(text: string): { cleanText: string; imageUrls: string[] } {
    const imageUrls: string[] = [];
    // Match markdown images: ![alt](url)
    const mdImageRe = /!\[[^\]]*\]\((\s*https?:\/\/[^\s)]+)\)/gi;
    // Match bare image URLs (must have image extension or known image hosts)
    const bareUrlRe = /(?<!\()https?:\/\/[^\s<>"')\]]+\.(?:jpg|jpeg|png|gif|webp|bmp)(?:\?[^\s<>"')\]]*)?/gi;
    // Known image hosting patterns (no extension needed)
    const hostPatterns = [
    ...
    cleanText = cleanText.replace(/!\[[^\]]*\]\(\s*https?:\/\/[^\s)]+\)/gi, '').trim();

    // Extract bare image URLs
    while ((match = bareUrlRe.exec(cleanText)) !== null) {
...
export const qqChannel: ChannelPlugin<ResolvedQQAccount> = {
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            // Warn about orphaned queue entries from previous generation
            let orphanedMsgCount = 0;
            for (const [qKey, qVal] of sessionQueues.entries()) {
                if (qVal.pendingPayloads.length > 0 || qVal.isProcessing) {
                    orphanedMsgCount += qVal.pendingPayloads.length + (qVal.isProcessing ? 1 : 0);
                    console.log(`[QQ][WARN:queue-orphan] Generation changed. Queue "${qKey}" has ${qVal.pendingPayloads.length} pending + isProcessing=${qVal.isProcessing}. These messages will be lost.`);
                }
            }
            if (orphanedMsgCount > 0) {
                console.log(`[QQ][WARN:queue-orphan] Total ~${orphanedMsgCount} message(s) orphaned by generation change to gen=${accountGen}.`);
                sessionQueues.clear();
            }
            ...
            client.on("message", async (event) => {
                try {
                    if (isStaleGeneration()) {
                        ...
                        console.log(
                            `[QQ][DROP:stale-gen] Message dropped (gateway generation changed). type=${event.message_type ?? "-"} group=${event.group_id ?? "-"} user=${event.user_id ?? "-"} msg="${rawPreview}"`
                        );
                        return;
                    }
                    getQQRuntime().channel.activity.record({
                    ...
                    const savedMediaList: Array<{ path: string; contentType?: string | null }> = [];
                    ...
                    if (Array.isArray(event.message)) {
                        ...
                        for (const seg of event.message) {
                            if (seg.type === "text") resolvedText += seg.data?.text || "";
                                ...
                                // Download image and save to disk for vision pipeline
                                if (imageUrl && imageUrl.startsWith("http")) {
                                    try {
                                        ...
                                        if (imgResp.ok) {
                                            ...
                                            const saved = await getQQRuntime().channel.media.saveMediaBuffer(buf, mimeType, "inbound");
                                            savedMediaList.push({ path: saved.path, contentType: mimeType });
                                            imageUrl = saved.path;
                                            console.log(`[QQ] Image saved to disk: ${saved.path} (${mimeType}, ${buf.length} bytes)`);
                                        }
                                    } catch (dlErr) {
                                        console.warn(`[QQ] Failed to download/save image for vision: ${String(dlErr)}`);
                                    }
                                }
                                if (imageUrl) {
                    ...
                    if (isGroup && /^\/models\b/i.test(inlineCommand)) {
                        ...
                        text = inlineCommand;
                        forceTriggered = true;
                    } else if (isGroup && /^\/(newsession|reset)\b/i.test(inlineCommand)) {
                        if (!isAdmin) return;
                        text = "/newsession";
                        forceTriggered = true;
                    }
                    ...
                    else if (/^\/voice\b/i.test(inlineCommand)) {
                        const voicePrompt = inlineCommand.replace(/^\/voice\s*/i, "").trim();
                        if (!voicePrompt) {
                            const usage = "[OpenClaw] /voice <提示词>\n示例:\n/voice 用日语介绍一下自己\n/voice 春日影についてどう思う？";
                            if (isGroup) client.sendGroupMsg(groupId, usage);
                            else client.sendPrivateMsg(userId, usage);
                            return;
                        }
                        text = voicePrompt;
                        forceTriggered = true;
...
/help - 帮助`;
                    ...
                    if (checkMention && config.requireMention && !isTriggered) {
                        ...
                        if (!effectiveSelfId) {
                            console.log(`[QQ][DROP:no-self-id] Cannot check mention — selfId not yet known. group=${groupId ?? "-"} user=${userId ?? "-"} msg="${text.slice(0, 100)}"`);
                            return;
                        }
                        if (Array.isArray(event.message)) {
                    ...
                    const deliver = async (payload: any) => {
                        ...
                        if (payload.text && payload.text.trim()) {
                            // Extract image URLs from text before sending
                            const { cleanText, imageUrls } = extractImageUrlsFromText(payload.text);

                            if (voiceRequested && !isGuild && config.enableSoVITS) {
                                // /voice mode: only send audio, skip text
                                try {
                                    const voiceMsg = await generateSoVITSAudio(payload.text, config, runtime?.logger);
                                    if (voiceMsg && !currentRunState?.isStale()) {
                                        if (isGroup) client.sendGroupMsg(groupId, voiceMsg);
                                        else client.sendPrivateMsg(userId, voiceMsg);
                                        deliveredAnything = true;
                                    } else {
                                        // TTS failed, fall back to text
                                        const sentText = await send(cleanText || payload.text);
                                ...
                                // Send text first (with image URLs removed)
                                const textToSend = cleanText.trim() || payload.text.trim();
                                const sentText = await send(textToSend);
                                if (sentText) deliveredAnything = true;
                            }

                            // Then send extracted images
                            for (const imgUrl of imageUrls) {
                                if (currentRunState?.isStale()) break;
                                await sendImage(imgUrl);
                            }
                        }
                        if (payload.files) {
                            for (const f of payload.files) {
                                ...
                                if (f.url) {
                                    ...
                                    if (currentRunState?.isStale()) return;
                                    const mimeIsImage = typeof f.mimeType === 'string' && f.mimeType.startsWith('image/');
                                    if (isImageFile(url) || isImageFile(f.url) || mimeIsImage) {
                                        await sendImage(url);
                    ...
                    // Build proper media payload from saved images (follows Feishu pattern)
                    const mediaPayload = savedMediaList.length > 0
                        ? buildAgentMediaPayload(savedMediaList)
                        : {};

                    // Fallback: any image URLs that weren't saved to disk
                    const fallbackMediaUrls = Array.from(new Set([
                        ...extractImageUrls(event.message),
                        ...imageHints.filter(u => !savedMediaList.some(m => m.path === u)),
                    ...
                    const ctxPayload = runtime.channel.reply.finalizeInboundContext({
                        ...
                        ...mediaPayload,
                        ...(fallbackMediaUrls.length > 0 && !mediaPayload.MediaUrls && { MediaUrls: fallbackMediaUrls }),
                        ...
                        try {
                            ...
                            for (let modelIndex = 0; modelIndex < modelsToTry.length; modelIndex++) {
                                ...
                                currentCfg = {
                                    ...
                                    // QQ channel safety: deny all server-side execution tools
                                    tools: {
                                        ...((cfg as any).tools || {}),
                                        deny: [
                    ...
                    console.log(`[QQ][MSG:ready] Passed all filters. group=${groupId ?? "-"} user=${userId ?? "-"} mentionByAt=${mentionedByAt} mentionByReply=${mentionedByReply} text="${text.slice(0, 80)}"`);
