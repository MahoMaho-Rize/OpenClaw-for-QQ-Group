...
export const QQConfigSchema = z.object({
  ...
  sessionCooldownMs: NumberInputSchema(20000).describe("同会话请求冷却时间（毫秒）。上一条消息开始处理后，冷却期内新 @ 消息直接丢弃。默认 20000（20秒）。0=关闭。"),
  ...
  enableSoVITS: BooleanInputSchema(false).describe("启用 GPT-SoVITS 语音合成。开启后每条文字回复会附带一条语音消息。"),
  sovitsApiUrl: z.preprocess((value) => normalizeLooseString(value), z.string().optional().default("http://127.0.0.1:9880")).describe("GPT-SoVITS API 地址。默认 http://127.0.0.1:9880"),
  sovitsRefAudioPath: z.preprocess((value) => normalizeLooseString(value), z.string().optional().default("")).describe("GPT-SoVITS 参考音频文件路径（服务器本地路径）。"),
  sovitsPromptText: z.preprocess((value) => normalizeLooseString(value), z.string().optional().default("")).describe("GPT-SoVITS 参考音频对应的提示文本。"),
  sovitsPromptLang: z.preprocess((value) => normalizeLooseString(value), z.string().optional().default("ja")).describe("GPT-SoVITS 参考音频语言。默认 ja（日语）。"),
  sovitsMaxChars: NumberInputSchema(300).describe("GPT-SoVITS 语音合成文本长度上限（字符）。超出则跳过语音。默认 300。"),
}).passthrough();
